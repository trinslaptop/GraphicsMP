...
/// 
/// MCPre supports a subset of C preprocessor directives and object macros.
...
/// 
/// User defined macros must not start with a `_`.
/// 
...
namespace mcpre {
	...
	// A very primitive preprocessor designed for use with GLSL shaders, supports `#include`, `#pragma once`, `#define` for `[A-Z_][A-Z0-9_]` object-like macros, `#undefine`, `#ifdef`, `#ifndef`, and `#endif`
	inline std::string preprocess(const std::string& path, std::map<std::string, std::string> macros = std::map<std::string, std::string>()) {
		try {
